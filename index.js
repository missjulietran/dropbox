  ...
  if (req.files) {
    ...
    file.mv(__dirname + "/uploaded/" + filename, (error) => {
      if (error) {
        ...
        Cache[filename] = file.data; // means we do not need to worry about sending anything but the data
        console.log(Cache);

        // let currentFiles = getCache(); -- I altered

        // res.send(currentFiles); -- I altered
...
app.get("/download/:name", (req, res) => {
  ...
  if (Cache[req.params.name]) {
    res.send(Cache[req.params.name]); // Alter this to not be .data
  } else {
    Cache[req.params.name] = fs.readFileSync(
      __dirname + "/uploaded/" + req.params.name
    );
    console.log(Cache[req.params.name]);
